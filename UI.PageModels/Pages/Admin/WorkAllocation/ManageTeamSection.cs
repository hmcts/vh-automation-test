...
public class ManageTeamSection : VhAdminWebPage
{
    ...
    private readonly By
        _restoreRowLocator =
            By.CssSelector("td:nth-child(7)"); // delete disappears and restore appears when a user has been deleted
    ...
    public void AddUserToTeam(string username, string firstName, string lastName, string contactTelephone,
        ...
        List<JusticeUserRoles> roles)
    {
        ...
        if (!string.IsNullOrWhiteSpace(contactTelephone)) EnterText(_justiceUserFormContactTelephone, contactTelephone);
    ...
    private IWebElement GetRowForByUsername(string username)
    {
        ...
        var locator =
            By.XPath(
                $"//tr[.//td[normalize-space()='{username}']] | //tr[.//td[normalize-space()='Deleted{username}']]");
